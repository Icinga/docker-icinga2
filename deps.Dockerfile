...
RUN ["/bin/bash", "-exo", "pipefail", "-c", "perl -pe s/buster/stretch/ < /etc/apt/sources.list > /etc/apt/sources.list.d/ssmtp.list"]

RUN ["/bin/bash", "-exo", "pipefail", "-c", "apt-get update; apt-get install --no-install-{recommends,suggests} -y dumb-init libboost-{context,coroutine,date-time,filesystem,program-options,regex,system,thread}1.67 libedit2 libmariadb3 libmoosex-role-timer-perl libpq5 libssl1.1 monitoring-plugins openssh-client openssl ssmtp; apt-get clean; rm -vrf /var/lib/apt/lists/*"]
